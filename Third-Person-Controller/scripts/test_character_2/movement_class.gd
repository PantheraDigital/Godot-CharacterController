...
var _enabled_movement: int = EnabledMovement.MOVE | EnabledMovement.IMPULSE | EnabledMovement.STEP
...
func move(direction: Vector3, speed: float) -> void:
	if (_enabled_movement & EnabledMovement.MOVE):
		if !direction.is_normalized():
			direction = direction.normalized()
		move_vector = direction * speed

func add_impulse(direction: Vector3, power: float) -> void:
	if (_enabled_movement & EnabledMovement.IMPULSE):
		if !direction.is_normalized():
			direction = direction.normalized()
		impulse_vector = direction * power
...
func _physics_process(delta: float) -> void:
	...
	if _animation_tree.get_root_motion_position() != Vector3.ZERO:
		...
		_character.velocity = Vector3(move_vector.x, _character.velocity.y, move_vector.z)
	
	if impulse_vector != Vector3.ZERO:
		_character.velocity = impulse_vector
	...
	if (_enabled_movement & EnabledMovement.STEP) and (!move_vector.is_zero_approx() or root_motion_override):
		...
		# perform ray cast in center
		var hit_result = CharacterStep3D.snapped_intersect_ray(_character.get_world_3d().direct_space_state, start, direction, full_width, false, [_character])
		if hit_result.has("normal"):
			found_ledge = hit_result.normal.angle_to(Vector3.UP) > _character.floor_max_angle
		# perform ray cast offset left
		if !found_ledge:
		...
		if found_ledge:
			...
			if !step_data.is_empty() and step_data["normal"].angle_to(Vector3.UP) <= _character.floor_max_angle:
				...
				var result_data: \
					...
					if hit_result.has("normal") else step_data
...
func disable_all_movement() -> void:
	disable_movement(EnabledMovement.MOVE | EnabledMovement.IMPULSE | EnabledMovement.STEP)

func enable_all_movement() -> void:
	enabled_movement(EnabledMovement.MOVE | EnabledMovement.IMPULSE | EnabledMovement.STEP)
