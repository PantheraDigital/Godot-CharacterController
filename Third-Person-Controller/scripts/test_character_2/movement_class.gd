...
func enable_all_movement() -> void:
	enable_movement(EnabledMovement.ALL)
...
func _step_up_correction() -> bool:
	var ledge_result = _step_search()
	if ledge_result == null:
		return false
	...
	var result_data: \
		Dictionary = {"step_point":step_data["point"], "step_normal":step_data["normal"], "ray_cast_point":ledge_result["position"], "ray_cast_normal":ledge_result["normal"]} \
		if ledge_result.has("normal") else {"step_point":step_data["point"], "step_normal":step_data["normal"]}
...
func _step_search():
	...
	return hit_result if found_ledge else null
